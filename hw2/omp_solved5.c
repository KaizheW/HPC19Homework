...
#pragma omp parallel shared(a, b, nthreads, locka, lockb) private(tid)
  ...
  #pragma omp sections nowait
    {
    // Comments:
    // In the two sections, the two locks (locka and lockb) should have the same
    // order. Otherwise, two sections are both locked and the program paused.
    #pragma omp section
      {
    ...
    #pragma omp section
      ...
      omp_set_lock(&locka); // Change lockb to locka
      for (i=0; i<N; i++)
        b[i] = i * PI;
      omp_set_lock(&lockb); // Change locka to lockb
      printf("Thread %d adding b[] to a[]\n",tid);
      for (i=0; i<N; i++)
        a[i] += b[i];
      omp_unset_lock(&lockb); // Change locka to lockb
      omp_unset_lock(&locka); // Change lockb to locka
      }
    }  /* end of sections */
